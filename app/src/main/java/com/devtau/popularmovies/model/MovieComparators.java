...
public class MovieComparators {
    public static Comparator<Movie> FIRST_FRESH = (first, second) -> {
    ...
    public static Comparator<Movie> FIRST_OLD = (first, second) -> {
        long firstLong = first.getDate().getTimeInMillis();
        long secondLong = second.getDate().getTimeInMillis();
        int result;
    ...
    public static Comparator<Movie> FIRST_HIGHER_PRICE =
            (first, second) -> second.getPrice() - first.getPrice();
    public static Comparator<Movie> FIRST_LOWER_PRICE =
            (first, second) -> first.getPrice() - second.getPrice();
    public static Comparator<Movie> ALPHABETICAL =
            (first, second) -> first.getDescription().compareTo(second.getDescription());
    public static Comparator<Movie> REV_ALPHABETICAL =
            (first, second) -> second.getDescription().compareTo(first.getDescription());

    //альтернатива без лямбды
//        public static Comparator<Movie> FIRST_LOWER_PRICE = new Comparator<Movie>() {
//            @Override
